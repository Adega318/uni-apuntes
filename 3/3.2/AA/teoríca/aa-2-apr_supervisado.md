...
Por lo tanto, cuando la dimensión VC disminuye la confianza VC disminuye indicando mayor poder de generalización a mayor riesgo, por lo contrario, al disminuir $n/h$ la confianza VC aumenta, llevando a sobre ajuste.
...
- Normalización, creación de una escala general para facilitar el aprendizaje.
	- Min/max, sé plasman los valores en un rango.
	  $\Large v'=\frac{v-min}{max-min}(nuevomax-nuevomin)+nuevomin$
	- Escalado decimal, sé mueve el punto decimal en los valores del atributo. 
	  $\Large v'=\frac{v}{10^{j}}$
	- Media cero, normalización para datos con algún dato fuera de lo normal.
	  $\Large v'=\frac{v-\mu}{\sigma}$
